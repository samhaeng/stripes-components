...
const convertValuesToPixels = (value, containerWidth) => {
  const htmlFontSize = parseInt(
    window.getComputedStyle(document.querySelector('html'), null)
  ...
  switch (unit) {
    case 'percent':
      return width * 0.01 * containerWidth;
    case 'vw':
      return width * 0.01 * windowWidth;
    case 'px':
      return width;
    case 'em':
    case 'rem':
      return width * htmlFontSize;
    default:
      return width;
  }
};
...
class MCLRenderer extends React.Component {
  ...
  measureColumns = ({
    ...
    columns.forEach((colName) => {
      if (typeof columnWidths !== 'undefined' &&
        ...
        cellWidths[colName] = convertValuesToPixels(columnWidths[colName], this.container.offsetWidth);
      } else {
    ...
    this.setState({
      rowWidth: Math.floor(Object.keys(cellWidths).reduce((acc, key) => (acc + cellWidths[key]), 0))
    });
  ...
  getRowWidth = () => {
    const { columnWidths, rowWidth } = this.state;

    if (typeof columnWidths === 'object' && !rowWidth) {
