...
class MCLRenderer extends React.Component {
  constructor(props) {
    ...
    this.initColumnWidths = this.initColumnWidths.bind(this);
    ...
    this.state = {
      ...
      loading: false,
      columns,
      columnWidths: this.initColumnWidths(columns),
      averageRowHeight: 0,
      adjustedHeight: null,
    };
  ...
  initColumnWidths(columns) {
    if(!this.props.contentData || !this.props.contentData.length) {
      return {};
    } else if(this.props.contentData.length > 0) {
  ...
  generateHeaders() {
    ...
    this.state.columns.forEach((header) => {
      ...
      headers.push(
        <div key={`header-${header}`} role="columnheader" className={this.getHeaderClassName(header)} style={headerStyle}>
          <div role="presentation" onClick={(e) => { this.handleHeaderClick(e, header); }} >
  ...
  render() {
    ...
    const { contentData, isEmptyMessage, rowMetadata, width, columnMapping } = this.props;
